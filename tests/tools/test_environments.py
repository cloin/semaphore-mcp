...
class TestEnvironmentTools:
    ...
    async def test_list_environments(self, environment_tools):
        ...
        project_id = 1
        mock_environments = [
            {"id": 1, "name": "Staging", "project_id": project_id, "json": "{\"STAGE\": \"staging\"}"},
            {"id": 2, "name": "Production", "project_id": project_id, "json": "{\"STAGE\": \"production\"}"}
        ]
    ...
    async def test_create_environment(self, environment_tools):
        ...
        environment_tools.semaphore.create_environment.assert_called_once_with(project_id, name, env_data)
    ...
    async def test_update_environment(self, environment_tools):
        ...
        # Call the method
        result = await environment_tools.update_environment(project_id, environment_id, name, env_data)
        
        # Verify the result
        assert result == mock_updated
        environment_tools.semaphore.update_environment.assert_called_once_with(project_id, environment_id, name, env_data)
    ...
    async def test_create_inventory(self, environment_tools):
        ...
        # Call the method
        result = await environment_tools.create_inventory(project_id, name, inventory_data)
        
        # Verify the result
        assert result == mock_created
        environment_tools.semaphore.create_inventory.assert_called_once_with(project_id, name, inventory_data)
    ...
    async def test_update_inventory(self, environment_tools):
        ...
        inventory_data = "[updated-webservers]\nlocalhost\n\n[updated-databases]\n127.0.0.1"
        mock_updated = {
            "id": inventory_id, 
            "name": "Updated Inventory", 
            "project_id": project_id,
        ...
        # Call the method
        result = await environment_tools.update_inventory(project_id, inventory_id, name, inventory_data)
        
        # Verify the result
        assert result == mock_updated
        environment_tools.semaphore.update_inventory.assert_called_once_with(project_id, inventory_id, name, inventory_data)
    ...
    async def test_delete_inventory(self, environment_tools):
        ...
        # Verify the result
        assert result == mock_result
        environment_tools.semaphore.delete_inventory.assert_called_once_with(project_id, inventory_id)
    ...
    async def test_list_environments_error(self, environment_tools):
        ...
        project_id = 1
        environment_tools.semaphore.list_environments.side_effect = Exception("API error")
    ...
    async def test_create_environment_error(self, environment_tools):
        ...
        name = "Test Env"
        env_data = {"VAR": "value"}
        environment_tools.semaphore.create_environment.side_effect = Exception("API error")
    ...
    async def test_update_environment_partial(self, environment_tools):
        ...
        result = await environment_tools.update_environment(project_id, environment_id, name=name)
        
        # Verify the result
        assert result == mock_updated
        environment_tools.semaphore.update_environment.assert_called_once_with(project_id, environment_id, name, None)
    ...
    async def test_update_inventory_partial(self, environment_tools):
        ...
        result = await environment_tools.update_inventory(project_id, inventory_id, inventory_data=inventory_data)
        
        # Verify the result
        assert result == mock_updated
        environment_tools.semaphore.update_inventory.assert_called_once_with(project_id, inventory_id, None, inventory_data)
    ...
    async def test_create_inventory_error(self, environment_tools):
        ...
        name = "Test Inventory"
        inventory_data = "[test]\nlocalhost"
        environment_tools.semaphore.create_inventory.side_effect = Exception("API error")
