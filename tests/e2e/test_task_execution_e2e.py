...
NOTE: These tests require access key management which is not yet implemented
in the MCP tools. The tests are skipped until access key tools are added.
To run these tests locally, you need to manually create an access key of type
"none" for the project first.
"""
...
# Skip all tests in this module - access key management not yet implemented
pytestmark = pytest.mark.skip(
    reason="Task execution tests require access key management tools (not yet implemented)"
)
...
class TestTaskExecutionE2E:
    ...
    def test_run_task_with_environment(
        self, inspector: MCPInspector, task_test_template: tuple
    ):
        """Test running a task with environment variable overrides.

        Uses the sleep_seconds and body variables from the test playbook.
        """
        template, project_id = task_test_template
        ...
        result = inspector.call_tool(
            "run_task",
            {
                "template_id": template["id"],
                "project_id": project_id,
                "environment": {"body": "Hello from E2E test"},
    ...
    def test_get_task_raw_output(
        ...
        wait_for_task_completion(inspector, project_id, task_id)
    ...
    def test_stop_task(self, inspector: MCPInspector, task_test_template: tuple):
        ...
        run_result = inspector.call_tool(
            "run_task",
            {
                "template_id": template["id"],
                "project_id": project_id,
                "environment": {"sleep_seconds": "60"},  # Long sleep so we can stop it
            },
        )
        run_data = parse_mcp_response(run_result)
        task_id = run_data["task"]["id"]
